...
# commented out by jrs on 13Feb2015 because it fouls up code text within
# the bc. and bc.. commands.
...
# jrs I made the following changes on 27Feb2015.
# I commented out the lines below and replaced them with pctw and pcth variables.
#        if ($w && $h) {
        if ($pctw && $pcth) {
#            if ($pctw || $pcth) {
#                $w = int($w * $pctw / 100);
...
#            }
            if ($self->{flavor} !~ m/^xhtml2/) {
#                $tag .= qq{ height="$h%" width="$w%"};
                $tag .= qq{ height="$pcth%" width="$pctw%"};
            } else {
#                $style .= qq{;height:$h%}.qq{px;width:$w%}.q{px};
                $style .= qq{;height:$pcth%}.qq{px;width:$pctw%}.q{px};
            }
        }
    }
    my $attr = $self->format_classstyle($clsty, $class, $style);
